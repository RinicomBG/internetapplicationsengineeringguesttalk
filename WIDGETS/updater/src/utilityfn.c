...
/**
 * Double to ASCII
 * Return value is the character count (not including '\0')
 */
...
static void dump_bytes(char * dest, char * prfx, char * bytes, int len) {
	...
	int count = 0;
	int offs = 0;
	// If a prefix on each line is required... use something like this.
	//size_t prfxlen;
	//prfxlen = strlen(prfx);
	...
	for (i = 0; i < len; i++) {
		...
		Utility_intToHex(dest + offs, (const unsigned char *)&bytes[i], 1);
		offs += 2;
		*(char *)(dest + offs) = ' ';
		offs++;
		count ++;
...
// len is the number of bytes that we want to dump
// each line is takes:
//   <HEX_CHAR> <HEX_CHAR> <PRINTABLE_CHARS>\r\n
//   LINE_LENGTH * 3 + LINE_LENGTH + 2
size_t Utility_dumphexBufferSize(size_t len) {
	size_t lines = (len / LINE_LENGTH) + 1;
	size_t size_in_bytes = lines * (LINE_LENGTH * 3 + LINE_LENGTH + 2);
...
char * Utility_easyDump(void * source, size_t source_len) {
	char * buffer;
	buffer = malloc(Utility_dumphexBufferSize(source_len));
	if (buffer == NULL) {
		return NULL;
	}
	Utility_dumphex(buffer, (const char *)source, source_len);
	return buffer;
}
...
// Read a string of hex digits with spaces or not :-)
